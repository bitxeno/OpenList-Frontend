...
const Preview = () => {
  ...
  const files = createMemo(() => sortedObjs().filter((obj) => !obj.is_dir))

  const previews = createMemo(() => {
    const file = files().find((f) => f.name === selectedFile())
    if (!file) return []

    return getPreviews({ ...file, provider: objStore.provider }).filter(
      (p) => p.availableInArchive !== false,
    )
  })
  ...
  const changeFile = (name: string) => {
    batch(() => {
      if (name === "") {
        ...
        if (file) {
          const objWithInner = buildObjWithInner(file)
          const innerUrl = rawLink(objWithInner)
          ObjStore.setObj(objWithInner)
